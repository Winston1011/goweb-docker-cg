...
import (
	...
	"goweb-docker-cg/components"
	"goweb-docker-cg/helpers"
...
sql示例：
INSERT INTO `demo` (`name`,`desc`,`del_flag`) VALUES ('goweb-docker-cg','this is goweb-docker-cg desc',0)
*/
...
VALUES ('goweb-docker-cg','this is TestUpsertIgnore desc',0,5)
...
// 获取第一条记录（主键升序）
db.First(&demo)
SELECT * FROM `demo` WHERE name = 'goweb-docker-cg' ORDER BY `demo`.`id` LIMIT 1;

// 获取一条记录，没有指定排序字段
db.Take(&demo)
SELECT * FROM `demo` WHERE name = 'goweb-docker-cg';

// 获取最后一条记录（主键降序）
db.Last(&demo)
SELECT * FROM `demo` WHERE name = 'goweb-docker-cg' ORDER BY `demo`.`id` DESC LIMIT 1;
...
sql示例：
SELECT * FROM `demo` WHERE name in ('goweb-docker-cg','demo_0')
*/
