...
func TestInsert(t *testing.T) {
	name := "goweb-docker-cg"
	desc := "this is goweb-docker-cg desc"
	d := &demo.Demo{
		Name: name,
		Desc: desc,
	}
...
func TestSelectAll(t *testing.T) {
	names := []string{"goweb-docker-cg", "demo_0"}
...
SELECT * FROM `demo` WHERE `id` = 2 AND `name` = 'goweb-docker-cg'
*/
func TestDemoInfo(t *testing.T) {
	ts, err := demo.Info(ctx, demo.WithID(2), demo.WithName("goweb-docker-cg"))
	if err != nil {
		t.Errorf("err want to got nil, got error(%s)", err.Error())
		return
	}
