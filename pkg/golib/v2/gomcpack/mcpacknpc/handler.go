...
import (
	...
	"goweb-docker-cg/pkg/golib/v2/gomcpack/mcpack"
	"goweb-docker-cg/pkg/golib/v2/gomcpack/npc"
...
func (h *Handler) Serve(w npc.ResponseWriter, r *npc.Request) {
	...
	if err := h.readRequest(r, argv.Interface()); err != nil {
		zlog.Warnf(nil, "readRequest: %v", err)
		return
	}
	if argIsValue {
		argv = argv.Elem()
	...
	if errInter != nil {
		zlog.Warnf(nil, "function call: %v", errInter.(error).Error())
		return
	}
	if err := h.sendResponse(w, replyv.Interface()); err != nil {
		zlog.Warn(nil, "sendResponse: ", err.Error())
		return
	}
}
