...
type Message interface {
	...
	GetCtxHeaders() map[string]string
...
func (m *messageWrapper) Send(ctx *gin.Context) (msgID string, err error) {
	...
	// 从 request ctx 或其他 ctx 获取 "X-Zyb-Ctx-" 开头 header，透传至 mq
	for k, v := range utils.GetTransportHeader(ctx) {
		m.msg.WithProperty(k, v)
	}
	...
	fields = append(fields, []zlog.Field{
		...
		//zlog.String("offsetMsgID", offset),
		...
		//zlog.String("ctxHeaders", fmtHeaders(m.msg, utils.ZYBTransportHeader)),
...
// GetCtxHeaders 返回 ctx headers
func (m *messageWrapper) GetCtxHeaders() map[string]string {
	headers := make(map[string]string)
	for key, value := range m.msg.GetProperties() {
		if strings.HasPrefix(key, utils.ZYBTransportHeader) {
			headers[header2Key(key)] = value
		}
	}
	return headers
...
func (batch MessageBatch) Send(ctx *gin.Context) (msgID string, err error) {
	...
	fields := []zlog.Field{
		...
		zlog.String("queue", queue),
		// use offsetid as msgID because the message id of batch send return is empty.
