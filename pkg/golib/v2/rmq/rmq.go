...
import (
	...
	"goweb-docker-cg/pkg/golib/v2/rmq/auth"
)
...
func (conf *ConsumerConf) Check() error {
	...
	if conf.Auth.AccessKey == "" && env.Namespace != "" && env.ServiceName != "" {
		id := auth.NewIdentity(env.Namespace+"_"+env.ServiceName, conf.Group, auth.ClientTypeConsumer)
...
func (conf *ProducerConf) Check() error {
	...
	// 未配置 ak 且添加了相关环境变量则会自动生成 aksk
	if conf.Auth.AccessKey == "" && env.Namespace != "" && env.ServiceName != "" {
		id := auth.NewIdentity(env.Namespace+"_"+env.ServiceName, "DEFAULT_PRODUCER", auth.ClientTypeProducer)
		conf.Auth.AccessKey, conf.Auth.SecretKey = id.Credential()
	}
